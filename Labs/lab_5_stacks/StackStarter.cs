...
namespace StackLab
{
    ...
    class StudentSkeleton
    {
        ...
        static void DisplayMenu()
        {
            ...
            Console.WriteLine($"   • Stack size: {actionHistory.Count} | Undo size: {undoHistory.Count} | Total ops: {totalOperations}");
        ...
        static void HandlePush()
        {
            ...
            actionHistory.Push(input);// to push new action
            undoHistory.Clear(); //new action -> redo history is not valid anymore
            totalOperations++;

            Console.WriteLine($"Pushed: \"{input}\" (top now = {actionHistory.Peek()})\n");
        ...
        static void HandlePop()
        {
            ...
            undoHistory.Push(popped);  // the removal can be undone
            totalOperations++;
        ...
        static void HandlePeek()
        {
            ...
            if (actionHistory.Count == 0)
            {
                Console.WriteLine("Nothing on the stack.\n");
                return;
            }

            string top = actionHistory.Peek();
            Console.WriteLine($"Top of stack: \"{top}\" (no change)\n");
        }
        ...
        static void HandleDisplay()
        {
            ...
            int indexFromTop = actionHistory.Count; // foreach goes top from bottom for Stack<T>
            foreach (var item in actionHistory)
            {
                string marker = (indexFromTop == actionHistory.Count) ? "►" : " ";
                Console.WriteLine($" {marker} {indexFromTop}. \"{item}\"");
                indexFromTop--;
            }
        ...
        static void HandleClear()
        {
            ...
            int count = actionHistory.Count + undoHistory.Count;
            if (count == 0)
            {
            ...
            undoHistory.Clear();
            totalOperations++;
            Console.WriteLine($"Cleared {count} items (stack + undo).\n");
        } 
        ...
        static void HandleUndo()
        {
            ...
            Console.WriteLine($"Undid last removal: \"{restored}\" (restored to stack)\n");
        }
        ...
        static void HandleRedo()
        {
            ...
            string removedAgain = actionHistory.Pop();
            undoHistory.Push(removedAgain);
            totalOperations++;

            Console.WriteLine($"Redo: removed \"{removedAgain}\" again (stored for Undo)\n");
        }
        ...
        static void ShowStatistics()
        {
            ...
            Console.WriteLine($"   Top action: {(actionHistory.Count > 0 ? $"\"{actionHistory.Peek()}\"" : "(none)")}\n");
        }
        ...
        static void ShowSessionSummary()
        {
            ...
            if (actionHistory.Count > 0)
            {
                Console.WriteLine("Remaining actions (top to bottom):");
                foreach (var item in actionHistory)
                {
            ...
            Console.WriteLine("\n LIFO brain unlocked. Press any key to exit.");
            Console.ReadKey();
        }
    }
