...
namespace FileSystemNavigator
{
    ...
    public class FileSystemBST
    {
        ...
        public List<FileNode> FindFilesByExtension(string extension)
        {
            ...
            // no tree or bad input → just return empty list
            if (root == null || string.IsNullOrWhiteSpace(extension))
                return results;

            // normalize extension: always like ".txt"
            extension = extension.Trim();
            if (!extension.StartsWith("."))
                extension = "." + extension;

            string targetExt = extension.ToLower();

            // traverse tree and collect only files with that extension
            TraverseAndCollect(root, results, f =>
                f.Type == FileType.File &&
                !string.IsNullOrEmpty(f.Extension) &&
                f.Extension.Equals(targetExt, StringComparison.OrdinalIgnoreCase)
            );
        ...
        private TreeNode? InsertNode(TreeNode? node, FileNode fileData)
        {
            ...
            // if we hit empty spot, create new node 
            if (node == null)
            {
                return new TreeNode(fileData);
            }

            // compare new file with current node
            int compare = CompareFileNodes(fileData, node.FileData);

            if (compare < 0)
            {
                // new item to the left side
                node.Left = InsertNode(node.Left, fileData);
            }
            else if (compare > 0)
            {
                // new item to the right side
